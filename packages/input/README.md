...
- 명세(SSOT): [input.md](../../docs/specs/input.md) (수용 기준 I-1 ~ I-6)
...
- **좌우(DAS/ARR)**: keydown 즉시 1회 → `das` 후 `arr` 간격 반복. `arr === 0`이면 `das`
  시점에 `moveToWall` 1회. 반대키는 **last-input 우선**, 남은 키로 복귀 시 **DAS 재충전**.
...
- **소프트드롭(SDF)**: `Infinity`면 `softDropToFloor()` 1회(홀드 중 락 이후 새 미노에 즉시 재적용).
...
수용 기준 대응: I-1~I-6 `core.test.ts` · 어댑터(I-2 repeat·I-3 blur) `dom.test.ts` ·
`EngineControls` 구조적 호환 `engine-compat.test.ts`.
