...
const renderer = new BoardRenderer(canvas, {
  ...
  theme: { highlight: "#66d9ff" }, // 부분 오버라이드 — 나머지는 기본 테마 (highlight = 외곽선 색)
});
...
renderer.render({
  ...
  overlays: { highlights: pageState.overlays?.highlights }, // "_"=없음, "H"=하이라이트 (외곽선 렌더 — 아래 테마 참조)
...
**`highlight`는 외곽선 색이다 (채움색 아님, RD-8).** 하이라이트 셀은 채우지 않고 셀 경계
안쪽(inside)에 `theme.highlight` 색 스트로크를 그린다 — 기본값은 불투명 흰색 `#ffffff`,
두께는 `max(1, round(cellSize / 8))`. 인접한 하이라이트끼리 맞닿은 변은 생략되어(오토
타일링, RD-9) 하이라이트 묶음의 바깥 윤곽만 남는다. `renderThumbnail`도 동일 규범을 따른다.
...
`describe`에 수용 기준 ID(RD-1 ~ RD-9)를 명시한다.
