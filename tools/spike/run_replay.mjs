...
// test/engine/replay.ts의 convert()를 JS로 포팅 (스파이크 보고서 §3 청사진)
...
const run = (opts, events, opponents, label) => {
  const frames = splitFrames(events);
  const engine = new Engine(convert(opts, opponents));
  try {
    while (engine.frame < frames.length) {
      engine.tick(frames[engine.frame]);
      if (engine.toppedOut) break;
    }
    console.log(`  [${label}] 재생 완료: ${engine.frame}프레임 | 배치 ${engine.stats.pieces}개 | toppedOut=${engine.toppedOut}`);
    return true;
  } catch (e) {
    console.log(`  [${label}] 실패 @프레임 ${engine.frame}: ${e.message}`); return false;
  }
};

const runTTRM = (ttrm, label) => {
  console.log(label);
  ttrm.replay.rounds.forEach((round, ri) => {
    for (const entry of round) {
      const opp = round.filter(r => r !== entry).map(r => r.replay.options.gameid);
      run(entry.replay.options, entry.replay.events, opp, `라운드 ${ri} · ${entry.username}`);
    }
  });
};

const ttrm = JSON.parse(fs.readFileSync("/mnt/user-data/uploads/replay_sample.ttrm"));
runTTRM(ttrm, "== ttrm 전체 (3라운드 × 2인) ==");

// R-3: 라운드 발췌 재조립 (라운드 1만 남긴 새 ttrm)
const excerpt = { ...ttrm, replay: { ...ttrm.replay, rounds: [ttrm.replay.rounds[1]] } };
fs.writeFileSync("excerpt.ttrm", JSON.stringify(excerpt));
runTTRM(JSON.parse(fs.readFileSync("excerpt.ttrm")), "== 발췌본 (원본 라운드 1만) ==");

const ttr = JSON.parse(fs.readFileSync("/mnt/user-data/uploads/sprint_rep_sample.ttr"));
console.log("== ttr (40L 스프린트) ==");
run(ttr.replay.options, ttr.replay.events, null, `${ttr.users[0].username} · ${ttr.gamemode}`);
